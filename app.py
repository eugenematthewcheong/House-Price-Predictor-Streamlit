...
def main():
    ...
    input_flat_type = right.selectbox('Select Flat Type',('1 ROOM', '2 ROOM', '3 ROOM', '4 ROOM', '5 ROOM', 'EXECUTIVE', 'MULTI-GENERATION'))
    ...
    input_floor_area_sqft = left.slider('Enter preferred floor area (square feet)',  step=1.0, format='%.2f',min_value=0.0, max_value=5000.0, value= 1000.8)
    input_mid = right.slider('Enter preferred floor in the block', step =1,format="%f",min_value=1, max_value=50, value=8)
    input_max_floor_lvl = left.number_input('Preferred maximum floor of the block', step=1.0, format='%.1f', value=25.0)
    input_tenure = right.slider('Enter preferred tenure', step =1,format="%f",min_value=1, max_value=99, value=8)
    input_transaction_year = left.slider('Input Transaction Year', step =1,format="%f",min_value=2012, max_value=2021, value=2013)
    input_mrt_nearest_distance = right.slider('Preferred distance to MRT (metres)',  step=1.0, format='%.2f',min_value=0.0, max_value=10000.0, value= 400.8)
    input_mh = left.slider('Enter preferred distance to Mall (metres)', step=1.0, format='%.2f',min_value=1.0, max_value=4000.0, value= 1000.8)
    input_from_centre_distance = st.slider('Preferred distance from CBD (metres)',  step=1.0, format='%.2f',min_value=0.0, max_value=35000.0, value= 400.8)
